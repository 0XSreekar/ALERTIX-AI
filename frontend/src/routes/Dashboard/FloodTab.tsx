...
import { floodEventsWs } from "@/lib/ws";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { FloodPrediction, HazardEvent } from "@/lib/types";
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <>
            <RiskGauge value={risk * 100} label="Flood Risk" className="mb-2" />
            {gfhAgrees != null && (
...
export default function FloodTab() {
  ...
  useEffect(() => {
    const unsub = floodEventsWs.subscribe((event) => {
      setLiveEvents((prev) => [event, ...prev].slice(0, 50));
    });
    return unsub;
  }, []);

  const allEvents = [...liveEvents, ...(eventsData?.events ?? [])];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <WsStatusBadge ws={floodEventsWs} label="Live" />
      </div>
